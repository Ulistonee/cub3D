...
void				get_rotated(int keynumber, t_all *all)
{
	...
	{
		old_x = all->plr.dir.x;
		all->plr.dir.x = old_x * all->cosi - all->plr.dir.y * -all->sinu;
		all->plr.dir.y = old_x * -all->sinu + all->plr.dir.y * all->cosi;
		old_x = all->plr.plane.x;
		all->plr.plane.x = old_x * all->cosi - all->plr.plane.y * -all->sinu;
		all->plr.plane.y = old_x * -all->sinu + all->plr.plane.y * all->cosi;
	}
	if (keynumber == RIGHT)
	{
		old_x = all->plr.dir.x;
